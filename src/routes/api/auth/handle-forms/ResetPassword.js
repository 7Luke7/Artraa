...
import { retreiveCookie, logout_all_devices } from "../../utils";
import { pool } from "../../db";
...
export const resetPasswordAction = action(async (formData) => {
    ...
    try {
        ...
        const change_password = await client.query(`
            UPDATE "User"
            SET password = $2,
                salt = $3
            WHERE id = $1
        ...
        await redisDel(`password:reset:${rs}`)
        await logout_all_devices(user_id , client)
